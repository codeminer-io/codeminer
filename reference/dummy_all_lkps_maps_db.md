...
#> Writing lookup and mapping tables to Duckdb database at /tmp/Rtmpv7eqly/file18f97109f155.db
...
#> icd10_phecode
#> metadata_codeminer
#> Success! Connect to database with `con <- DBI::dbConnect(duckdb::duckdb(), '/tmp/Rtmpv7eqly/file18f97109f155.db', read_only = TRUE)`, then access all tables with `all_lkps_maps <- ukbwranglr::db_tables_to_list(con)`
```
