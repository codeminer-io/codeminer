...
#> Writing lookup and mapping tables to Duckdb database at /tmp/RtmpLYACwM/file1c676e8fd0e8.db
...
#> icd10_phecode
#> metadata_codeminer
#> Success! Connect to database with `con <- DBI::dbConnect(duckdb::duckdb(), '/tmp/RtmpLYACwM/file1c676e8fd0e8.db', read_only = TRUE)`, then access all tables with `all_lkps_maps <- ukbwranglr::db_tables_to_list(con)`
```
