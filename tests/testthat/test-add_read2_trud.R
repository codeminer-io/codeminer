...
test_that("add_read2_trud() adds lookup + relationship to database", {
  dir <- create_dummy_read2_dir()
  local_build_temp_database()
  ...
  con <- connect_to_db()
  expect_true(table_exists(con, "Read v2_test_v1"))
  expect_true(table_exists(con, "Read v2_relationship_test_v1"))
})
