...
codeminer stores everything in a single DuckDB file. The file holds the
data tables for each code system (lookups, mappings, relationships) plus
a small set of **metadata tables** that index them.
...
- The package declares `current_schema_version()` (what fresh
  [`build_database()`](https://codeminer-io.github.io/codeminer/reference/build_database.md)
  writes) and `min_readable_schema_version()` (the oldest schema this
  codeminer can still operate on).
- Every codeminer DB has a `_db_metadata` row stamping which schema
  ...
  provenance.
- On connect, codeminer compares the DB’s stamp to the package’s range
  and applies a gate.
...
| in `[min_readable, current)`, **any** registered migration is `manual_additive` or `breaking` | refuse with a clear pointer at [`migrate_database()`](https://codeminer-io.github.io/codeminer/reference/migrate_database.md); nothing silent |
...
`schema_version = 0` and walks the chain from there.
...
Append a record to the list returned by `codeminer_migrations()`
(`R/migrate.R`) and bump `current_schema_version()`:
...
Default to the strictest mode that fits — better to ask the user to
confirm than to silently rewrite data.

[`migrate_database()`](https://codeminer-io.github.io/codeminer/reference/migrate_database.md)
walks the chain from the DB’s stamp to `current_schema_version()`. It
refuses to run any non-auto migration without an explicit call (the
connect gate refuses outright). A `dry_run = TRUE` flag prints the
pending chain without applying it.

### The compatibility window

`min_readable_schema_version()` defines how far back this codeminer
supports. Older DBs are hard-refused with a pointer at the last
codeminer version that could read them. Bump `min_readable` only when
carrying support for an old schema becomes a maintenance burden — never
gratuitously, because doing so abandons users with DBs at older
versions.
