...
#'   is `FALSE`.
#'
#' @return File path to the downloaded OPCS-4 zip file (invisibly).
#'
#' @details
#' This function requires a valid NHS TRUD API key set as the environment
#' variable `TRUD_API_KEY`. You must also be subscribed to item 119 on the NHS
#' TRUD website.
#'
#' @seealso [read_opcs4_trud()], [add_opcs4_trud()]
#' @export
#' @examples
#' \dontrun{
#' path <- get_opcs4_trud()
...
get_opcs4_trud <- function(
  ...
  msg("Getting OPCS-4 (TRUD item {trud_item})")
  ...
  if (file.exists(target_zip) && !overwrite) {
    msg(c(
      "v" = "OPCS-4 already exists at {.file {target_zip}}.",
  ...
  msg("OPCS-4 downloaded to {.file {zip_filepath}}")
