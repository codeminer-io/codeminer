...
#'   is `FALSE`.
#'
#' @return File path to the downloaded ICD-10 zip file (invisibly).
#'
#' @details
#' This function requires a valid NHS TRUD API key set as the environment
#' variable `TRUD_API_KEY`. You must also be subscribed to item 258 on the NHS
#' TRUD website.
#'
#' @seealso [read_icd10_trud()], [add_icd10_trud()]
#' @export
#' @examples
#' \dontrun{
#' path <- get_icd10_trud()
...
get_icd10_trud <- function(
  ...
  msg("Getting ICD-10 Edition 5 (TRUD item {trud_item})")
  ...
  if (file.exists(target_zip) && !overwrite) {
    msg(c(
      "v" = "ICD-10 already exists at {.file {target_zip}}.",
  ...
  msg("ICD-10 downloaded to {.file {zip_filepath}}")
